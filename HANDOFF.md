...
- Phone numbers in `patient_master` are stored inconsistently formatted (some with dashes/parens, some blank, ~4,900 of the ~14,900 active rows are garbage — random 13-digit strings, repeated placeholder values, hash fragments — that were never real phone numbers to begin with). Matching uses `App\Helpers\Common::normalizePhoneNumberdate()` (strips to digits, drops a leading US `1`) against `PatientService::getPatientsByNormalizedPhone()`. **Changed 2026-08-17**: this used to be a `LIKE '%digits'` suffix match across `mobile`, `phone`, and `emergency_phone` — flagged by the user as a real risk (a wrong/partial number could resolve to a different patient's record via that open-ended suffix search). Now it's an **exact** match against a digit-stripped `mobile` column only (`phone`/`emergency_phone` dropped — confirmed via a DB query that zero active rows rely on them as a fallback when `mobile` is blank), plus one precise, length-gated exception for a stored value carrying a leading US country code (11 digits, `1` + the real 10) — mirrors `normalizePhoneNumberdate()`'s own logic exactly, not a general suffix search, so it doesn't reopen the garbage-data collision risk. **A single phone number can legitimately match multiple `patient_master` rows** (e.g. re-referrals, shared family numbers) — the whole API is written to handle a *set* of matched patient rows, not just one.
