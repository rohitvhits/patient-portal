{{-- Rendered server-side and injected into #appointment-detail-results (see show.blade.php).
     Needs $appointment, $appointmentAgencyName, $telehealth, $schedule, $documents,
     $patientIdentity, $appointmentUnavailable, $documentsUnavailable — all supplied by
     AppointmentController::showFragment(). --}}
@php
        ...
        <ul class="divide-y divide-slate-100">
            @foreach ($documents as $document)
                ...
                <li class="group flex flex-col gap-3 px-6 py-4 transition hover:bg-slate-50/70 hover:shadow-[inset_3px_0_0_var(--color-brand-500)] sm:flex-row sm:items-center sm:justify-between sm:px-8">
                    ...
                    <a href="{{ route('documents.download', $document) }}" data-no-loader aria-label="Download {{ $document->document_name ?: 'document' }}" class="inline-flex w-full shrink-0 items-center justify-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-semibold text-slate-700 shadow-sm outline-none transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-800 focus-visible:ring-2 focus-visible:ring-brand-500/40 active:scale-[0.97] sm:w-auto sm:py-2">
